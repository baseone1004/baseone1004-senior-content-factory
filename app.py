...
반드시 아래 형식으로 10개를 출력하세요. 특수문자 없이 작성하세요. 각 항목은 아래 형식을 정확히 따르세요:

번호/주제/출처뉴스/떡상확률/이유/추천태그/난이도/광고적합
...
- 떡상확률은 50에서 95 사이 숫자로 쓴다.
- 난이도는 쉬움 또는 보통 또는 어려움 중 하나를 쓴다.
- 광고적합은 반드시 적합으로만 쓴다.
- 10개 주제는 모두 서로 다른 각도여야 한다.
- 떡상확률이 높은 순서로 정렬한다.
- 슬래시로 구분하고 따옴표나 괄호나 중괄호 같은 특수문자를 절대 쓰지 않는다."""
